...
export default function MasterDashboard({ staff, onLogout }: { staff: StaffSession; onLogout: () => void }) {
  const [view, setView] = useState<MasterView>("overview"), [emails, setEmails] = useState<any[]>([]), [emailError, setEmailError] = useState(""), [creatorOpen, setCreatorOpen] = useState(false);
  const { last24Tasks, loading, error, reload } = useTasks(staff.name, true, true);
  const loadEmails = useCallback(async () => { try { setEmailError(""); setEmails(await fetchEmailReaderItems()); } catch (reason) { setEmailError(reason instanceof Error ? reason.message : "Unable to load email inbox."); } }, []);
  useEffect(() => { if (!["overview", "email"].includes(view)) return; void loadEmails(); const timer = window.setInterval(loadEmails, 60000); return () => window.clearInterval(timer); }, [view, loadEmails]);
  ...
  return <main className="staff-os master-os"><aside className="staff-rail"><div className="staff-brand"><span>N K</span><strong>Hotel <em>OS</em></strong></div><div className="master-rail-label">MASTER WORKSPACE</div><nav aria-label="Master workspace">{navigation.map(item => <button key={item.key} className={view === item.key ? "active" : ""} onClick={() => setView(item.key)}><span className={`nav-mark nav-${item.key}`} />{item.label}{item.key === "email" && emails.length > 0 && <b>{emails.length}</b>}</button>)}</nav><div className="staff-user"><span>{staff.name.slice(0,1).toUpperCase()}</span><div><strong>{staff.name}</strong><small>Master</small></div><button onClick={onLogout}>↗</button></div></aside><section className="staff-stage"><header className="staff-topbar master-topbar"><div><small>N K HOTELS COMMAND CENTER</small><h1>{navigation.find(item => item.key === view)?.label}</h1></div><div className="master-top-actions"><span><i />Master access</span><button className="primary-action" onClick={() => setCreatorOpen(true)}>＋ Create Task</button></div></header><div className="staff-content">
    ...
    {view === "email" && <EmailInbox items={emails} staff={staff} shift={shift} canUseTasks error={emailError} onRefresh={refreshAll} />}{view === "whatsapp" && <WhatsAppInbox onCreate={() => setCreatorOpen(true)} />}
