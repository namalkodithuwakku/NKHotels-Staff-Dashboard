...
type MasterView = "overview" | "tasks" | "scheduled" | "properties" | "staff" | "roster" | "calendar" | "whatsapp" | "sms" | "team-break" | "reports" | "settings";
const navigation: Array<{ key: MasterView; label: string }> = [
  ...
  { key: "roster", label: "Roster" }, { key: "calendar", label: "Calendars" },
...
export default function MasterDashboard({ staff, onLogout }: { staff: StaffSession; onLogout: () => void }) {
  ...
  return <main className="staff-os master-os"><aside className="staff-rail"><div className="staff-brand"><span>NKH</span><strong><em>Dashboard</em></strong></div><div className="master-rail-label">MASTER WORKSPACE</div><nav aria-label="Master workspace">{navigation.map(item => <button key={item.key} className={view === item.key ? "active" : ""} onClick={() => setView(item.key)}><span className={`nav-mark nav-${item.key}`} />{item.label}{Boolean(notificationCounts[item.key]) && <b>{notificationCounts[item.key]}</b>}</button>)}</nav><div className="staff-user"><span>{staff.name.slice(0,1).toUpperCase()}</span><div><strong>{staff.name}</strong><small>Master</small></div><button onClick={onLogout}>↗</button></div></aside><section className="staff-stage"><header className="staff-topbar master-topbar"><div><small>NKH COMMAND CENTER</small><h1>{navigation.find(item => item.key === view)?.label}</h1></div><div className="master-top-actions"><span><i />Master access</span><button className="primary-action" onClick={() => setCreatorOpen(true)}>＋ Create Task</button></div></header><div className="staff-content">
    ...
    {view === "properties" && <PropertiesWorkspace access="Master" />}{view === "staff" && <StaffProfilesWorkspace />}{view === "roster" && <RosterWorkspace />}{view === "calendar" && <CalendarWorkspace />}
