...
export default function RosterWorkspace() {
  ...
  return <div className="roster-workspace">
    <div className="roster-toolbar"><div><small>TEAM COVERAGE</small><h2>{view === "week" ? "Weekly roster" : "Monthly roster"}</h2><p>{periodLabel}</p></div><div className="roster-toolbar-actions"><div className="roster-view-toggle"><button className={view === "week" ? "active" : ""} onClick={() => setView("week")}>Week</button><button className={view === "month" ? "active" : ""} onClick={() => setView("month")}>Vertical month</button></div><button onClick={() => move(-1)}>‹ Previous</button><button onClick={() => setWeek(startOfWeek(new Date()))}>Today</button><button onClick={() => move(1)}>Next ›</button><button className="primary-action" onClick={() => openNew()}>＋ Add Shift</button></div></div>
    ...
    {view === "week" ? <div className="roster-grid">{days.map(date => { const dateKey = key(date), items = entries.filter(item => item.shift_date === dateKey); return <section className="roster-day" key={dateKey}><header><div><small>{date.toLocaleDateString("en-US", { weekday: "short" }).toUpperCase()}</small><strong>{date.getDate()}</strong></div><button onClick={() => openNew(dateKey)}>＋</button></header><div className="roster-day-entries">{items.length ? items.map(item => { const person = staffFor(item.staff_id), property = propertyFor(item.property_id); return <button className={`roster-entry ${item.status.toLowerCase()}`} key={item.id} style={{ borderLeftColor: person?.color_hex || "#E98A15" }} onClick={() => openEdit(item)}><strong>{person?.display_name || "Staff"}</strong>{item.status === "Scheduled" ? <><span>{timeLabel(item.start_time)} – {timeLabel(item.end_time)}</span><small>{property?.property_name || item.shift_label || "General coverage"}</small></> : <span>{item.status}</span>}</button>; }) : <div className="roster-no-cover"><span>○</span><small>No shifts</small></div>}</div></section>; })}</div> :
      <div className="roster-vertical" style={{ "--roster-staff-count": staff.length } as CSSProperties}><div className="roster-vertical-head"><span>Date</span>{staff.map(person => <strong key={person.id}>{person.display_name}</strong>)}</div>{monthDays.map(date => { const dateKey = key(date); return <div className="roster-vertical-row" key={dateKey}><button className="roster-vertical-date" onClick={() => openNew(dateKey)}><b>{date.getDate()}</b><span>{weekdays[date.getDay()]}</span></button>{staff.map(person => { const items = entries.filter(item => item.shift_date === dateKey && item.staff_id === person.id); return <div className="roster-vertical-cell" key={person.id}>{items.length ? items.map(item => <button key={item.id} className={item.status.toLowerCase()} onClick={() => openEdit(item)}>{item.status === "Scheduled" ? <><strong>{timeLabel(item.start_time)}–{timeLabel(item.end_time)}</strong><small>{propertyFor(item.property_id)?.property_name || item.shift_label || "Coverage"}</small></> : <strong>{item.status}</strong>}</button>) : <button className="empty" onClick={() => openNew(dateKey)}>＋</button>}</div>; })}</div>; })}</div>}
