...
export default function RevenuePlannerTool() {
  ...
  return <section className="revenue-planner">
    ...
    {result && <div className="revenue-plan-results">
      <div className="revenue-metrics"><article><small>ROOMS</small><strong>{result.metrics.inventory}</strong><span>Property inventory</span></article><article><small>AVERAGE OCCUPANCY</small><strong>{result.metrics.averageOccupancy}%</strong><span>Selected period</span></article><article><small>BOOKED REVENUE</small><strong>{new Intl.NumberFormat("en-US",{maximumFractionDigits:0}).format(result.metrics.bookedRevenue)}</strong><span>{result.metrics.currency}</span></article><article><small>SEASON</small><strong>{result.plan.season}</strong><span>{result.plan.demandOutlook}</span></article></div>
      <section className="revenue-summary"><small>AI PERIOD SUMMARY</small><h3>{result.plan.periodSummary}</h3><p>{result.plan.destinationSummary}</p></section>
