...
type StaffRow = { id:string; display_name:string|null; access:string|null; active:boolean|null };
...
export async function GET(request: NextRequest) {
  ...
  try {
    const [tasks, emails, staff] = await Promise.all([
      ...
      supabaseAdmin<StaffRow[]>("nkh_staff?select=id,display_name,access,active&order=display_name.asc&limit=100"),
    ]);
    const now = Date.now();
    ...
    return NextResponse.json({
      ...
      counts:{ open:open.length, urgent:urgent.length, inProgress:inProgress.length, overdue6h:overdue.length, stale24h:stale.length, unassigned:unassigned.length, emailQueue:emails.length, activeStaff:staff.filter(s=>s.active!==false).length },
      urgentTasks:urgent.slice(0,30),
      overdueTasks:overdue.slice(0,50),
      unassignedTasks:unassigned.slice(0,30),
      staff:staff.map(s=>({ id:s.id, name:s.display_name||"", access:s.access||"", active:s.active!==false })),
    });
  } catch (error) {
