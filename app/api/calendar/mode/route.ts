...
type RoomType = { id: string; room_code: string; room_name: string; room_count: number; display_order: number };
  ...
  try {
    ...
    if (mode === "supabase") {
      const roomTypes = await supabaseAdmin<RoomType[]>(
        `nkh_room_types?property_id=eq.${encodeURIComponent(propertyId)}&is_active=eq.true&select=id,room_code,room_name,room_count,display_order&order=display_order.asc`
      );
      ...
      const rooms = roomTypes.flatMap(roomType =>
        Array.from({ length: Number(roomType.room_count) }, (_, index) => ({
          ...
          room_name: `${roomType.room_name} ${index + 1}`,
