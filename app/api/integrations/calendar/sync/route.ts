...
export async function GET(request: NextRequest) {
  try {
    ...
    const properties = await supabaseAdmin<Array<{ id: string; client_code: string; property_name: string; calendar_sheet_code: string }>>(
      "nkh_properties?select=id,client_code,property_name,calendar_sheet_code&client_status=in.(Active,Onboarding)&calendar_sheet_code=not.is.null&order=property_name.asc"
    );
