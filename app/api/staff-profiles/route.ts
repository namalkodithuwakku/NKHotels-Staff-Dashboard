...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!validUsername(username)) return NextResponse.json({ error: "Username must be 3–40 lowercase letters, numbers, dots, dashes or underscores." }, { status: 400 });
...
export async function PATCH(request: NextRequest) {
  try {
    ...
    if (Object.prototype.hasOwnProperty.call(update, "login_username")) {
      ...
      if (!validUsername(String(update.login_username))) return NextResponse.json({ error: "Username must be 3–40 lowercase letters, numbers, dots, dashes or underscores." }, { status: 400 });
    }
    const pin = String(input.pin || "");
    if (pin) {
      if (!validPin(pin)) return NextResponse.json({ error: "PIN must contain 4–12 numbers." }, { status: 400 });
